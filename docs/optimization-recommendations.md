...
| 维度 | 当前状态 | 判断 |
|---|---|---|
| 包化迁移 | `paper_rag/` 已承载 generation、retrieval、observability、ui、config 等部分实现，根目录保留兼容薄壳和入口文件 | 已进入迁移收口阶段，不是从零开始 |
...
| 兼容薄壳 | `generation_service.py`、`context_builder.py`、`retrieval_router.py`、`source_utils.py` | 分阶段移除 |
| 仍有真实实现 | `hybrid_retriever.py`、`query_expansion.py`、`reranker.py`、`context_compression.py` | 先迁入包内，再保留薄壳 |

**建议阶段**

| 阶段 | 动作 | 验收 |
|---|---|---|
| 1 | 全局替换内部 import 为 `paper_rag.*` | 单测通过，benchmark schema 检查通过 |
| 2 | 对薄壳增加 deprecation 注释或测试保护 | 旧入口仍可用 |
| 3 | 将剩余真实实现迁入 `paper_rag.retrieval`、`paper_rag.generation` | 根目录只剩入口和兼容壳 |
